...
public class SyntheticDataGenerationScheme6 {

//	static final int[] arr_nRec = {10000, 15848};
//	static final int[] arr_nRule = {10000, 30000, 100000};
	static final int[] arr_nRec = {10000, 31622, 100000, 316227, 1_000_000, 3_162_277, 10_000_000};
...
//	static final int[] arr_id_SEL = {-4, -3, -2, -1};
	...
	static final int[] arr_NAR = {6};
	static final int[] arr_LCF = {1, 10, 20};

	static final double[] arr_KAP = {0, 0.5, 1.0};
	static final double[] arr_SEL = {1e-2};
	static final double[] arr_skewR = {0.0};
	static final double[] arr_skewP = {0.0};
	...
	static final String syn_id = "SYN6";
...
//		}
		...
		String rulePathTemplate = token[token.length-1].replaceFirst( DIC+"_"+nRuleMax+"_", DIC+"_%s_" );
		for ( int nRule : arr_nRule ) {
		...
		String recordPathTemplate = token[token.length-1].replaceFirst( syn_id+"_"+DIC+"_"+nRecMax+"_", syn_id+"_"+DIC+"_%s_" );
		...
		if (!selfJoin) {
			...
			token = recordPath.split( "/" );
			recordPathTemplate = token[token.length-1].replaceFirst( syn_id+"_"+DIC+"_"+nRecMax+"_", syn_id+"_"+DIC+"_%s_" );
			for ( int nRec : arr_nRec ) {
				if ( nRec == nRecMax ) break;
				BufferedReader reader = new BufferedReader( new FileReader( recordPath ) );
				BufferedWriter writer = new BufferedWriter( new FileWriter( parentPath+"/"+outputPath+"/data/"+String.format( recordPathTemplate, nRec ) ) );
...
//		}
		ps_run.println(
				"dict_path[\'"+dname+"\'] = \'../" + recordPath.replaceAll( syn_id+"_"+DIC+"_"+nRecMax+"_", syn_id+"_"+DIC+"_%s_" ).replaceAll( "_1.txt", "_%s.txt" ) + "\'\n" +
				"dict_path[\'"+dname+"_rule\'] = \'../" + rulePath.replaceAll( DIC+"_"+nRuleMax+"_", DIC+"_%s_" ) + "\'"
				);
