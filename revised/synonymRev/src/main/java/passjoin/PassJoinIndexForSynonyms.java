...
//		System.out.println( "init: "+(long)((afterInit - afterSort)/1e6) );
		prepare();
//		long afterPrepare = System.nanoTime();
//		System.out.println( "prepare: "+(long)((afterPrepare- afterInit)/1e6) );
		buildIndex();
...
//			if ( searchedList.get( id ).getID() == 681 ) debug = true;
			if (debug) SampleDataTest.inspect_record( searchedList.get( id ), query, 1 );
			if (debug) System.out.println( "searched ID: "+searchedList.get( id ).getID() );
...
//										if (debug) System.out.println( "d_lcs: "+Util.lcs(x, y, D, 0, 0, -1, -1) );
			...
			for ( int answer_id : answer_ids ) {
				Record rec1 = searchedList.get( id );
				Record rec2 = indexedList.get( answer_id );
				AlgorithmTemplate.addSeqResult( rec1, rec2, rslt, isSelfJoin );
