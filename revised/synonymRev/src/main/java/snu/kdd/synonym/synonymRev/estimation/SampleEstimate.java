...
public class SampleEstimate {
	...
	public void estimateJoinHybridWithSample( StatContainer stat, Validator checker, int indexK, int qSize ) {
		...
		estimateJoinMin( stat, checker, indexK, qSize );
		long tAfterMin = System.nanoTime();
		estimateJoinMH( stat, checker, indexK, qSize );
		long tAfterMH = System.nanoTime();
		estimateNaive( stat );
		long tAfterNaive = System.nanoTime();
		long estTimeNaive = tAfterNaive - tAfterMH;
		long estTimeJoinMH = tAfterMH - tAfterMin;
		long estTimeJoinMin = tAfterMin - ts;
		
		try {
			bw_log.write( "estTimeNaive\t"+estTimeNaive/1e6+"\n" );
			bw_log.write( "estTimeJoinMH\t"+estTimeJoinMH/1e6+"\n" );
			bw_log.write( "estTimeJoinMin\t"+estTimeJoinMin/1e6+"\n" );
		}
		catch (IOException e ) { e.printStackTrace(); }
...
//		long currentThreshold = Math.min( sampleSearchedList.get( 0 ).getEstNumTransformed(), sampleIndexedList.get( 0 ).getEstNumTransformed() );
		...
		// Number of bigrams generated by expanded TL records
		double searchedTotalSigCountMH = 0;
		double searchedTotalSigCount = 0;
		...
		for( int recId = 0; recId < tableSearchedSize; recId++ ) {
			...
			for( int i = 0; i < searchmax; ++i ) {
				...
				List<QGram> available = availableQGrams.get( i );
				searchedTotalSigCount += available.size();
				if ( i < indexK ) searchedTotalSigCountMH += available.size();
		...
		for( int recId = 0; recId < tableIndexedSize; recId++ ) {
			...
			for( int position = 0; position < availableQGrams.size(); position++ ) {
				...
				if ( position < indexK && minJoinMHComparison > comparison ) {
			...
			System.out.println( Arrays.toString( rec.getTokensArray() )+"\t"+minJoinMHComparison+"\t"+minJoinMinComparison+"\t"+totalJoinMHInvokes+"\t"+totalJoinMinInvokes );
		...
		Util.printLog( "searchedTotalSigCount: "+searchedTotalSigCount );
		Util.printLog( "searchedTotalSigCountMH: "+searchedTotalSigCountMH );
		Util.printLog( "indexedTotalSigCount: "+indexedTotalSigCount );
...
//				nextThreshold = maxThreshold + 1;
			...
			double joinmhEstimation = this.getEstimateJoinMH( searchedTotalSigCountMH, joinMHIndexedSigCount,
			...
			try {
				...
				bw_log.write( String.format( "%d\t%d\t%d\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t|\t", 
						(long)joinMHIndexedSigCount, (long)searchedTotalSigCountMH, (long)(totalJoinMHInvokes-removedJoinMHComparison), 
						gamma*joinMHIndexedSigCount/1e6, zeta*searchedTotalSigCountMH/1e6, eta*(totalJoinMHInvokes-removedJoinMHComparison)/1e6, joinmhEstimation/1e6,
						gamma*joinMHIndexedSigCount/sampleRatio/1e6, zeta*searchedTotalSigCountMH/sampleRatio/1e6, 
						eta*(totalJoinMHInvokes-removedJoinMHComparison)/sampleRatio/sampleRatio/1e6, 
						getEstimateJoinMH( searchedTotalSigCountMH/sampleRatio, joinMHIndexedSigCount/sampleRatio, (totalJoinMHInvokes-removedJoinMHComparison)/sampleRatio/sampleRatio)/1e6
						) );

				// BKP
				bw_log.write( String.format( "%d\t%d\t%d\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\t|\t", 
						(long)indexedTotalSigCount, (long)searchedTotalSigCount, (long)(totalJoinMinInvokes-removedJoinMinComparison), 
						lambda*indexedTotalSigCount/1e6, mu*searchedTotalSigCount/1e6, rho*(totalJoinMinInvokes-removedJoinMinComparison)/1e6, joinminEstimation/1e6,
						lambda*indexedTotalSigCount/sampleRatio/1e6, mu*searchedTotalSigCount/sampleRatio/1e6, 
						rho*(totalJoinMinInvokes-removedJoinMinComparison)/sampleRatio/sampleRatio/1e6, 
