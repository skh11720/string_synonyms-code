...
public class SampleEstimateDelta {
	...
	public void estimateNaiveDelta( StatContainer stat ) {
...
//		stat.add( "Stat_Coeff_Naive_3", coeff_naive_3 );
		...
		System.out.println( "AKLJSLKSAJLK" );
		System.out.println( coeff_naive_1 );
		System.out.println( coeff_naive_2 );
		System.out.println( coeff_naive_3 );
//		System.out.println( Arrays.toString( naive_term2 ) );
//		System.out.println( Arrays.toString( naive_term3 ) );
		System.out.println( "est time: "+getEstimateNaive( naive_term1, naive_term2[sampleSearchedList.size()-1], naive_term3[sampleSearchedList.size()-1] ) );
	}
	...
	public void estimateJoinMinDelta( StatContainer stat, Validator checker, int indexK, int qSize ) {
		...
		coeff_min_2 = ( joinmininst.indexCountTime + joinmininst.candQGramCountTime ) / joinmininst.searchedTotalSigCount;
		coeff_min_3 = ( joinTime - joinmininst.candQGramCountTime ) / joinmininst.predictCount;

		System.out.println( "KEJKWLNFKLWJD" );
		System.out.println( coeff_min_1 );
		System.out.println( coeff_min_2 );
		System.out.println( coeff_min_3 );
//		System.out.println( Arrays.toString( min_term2 ) );
//		System.out.println( Arrays.toString( min_term3 ) );
		System.out.println( "est time: "+getEstimateJoinMin( min_term1, min_term2[sampleSearchedList.size()-1], min_term3[sampleSearchedList.size()-1]) );
	}
	...
	public void estimateJoinMHDelta( StatContainer stat, Validator checker, int indexK, int qSize ) {
		...
		System.out.println( "DLJEIOMEOKLDJD" );
		System.out.println( coeff_mh_1 );
		System.out.println( coeff_mh_2 );
		System.out.println( coeff_mh_3 );
//		System.out.println( Arrays.toString( mh_term2 ) );
//		System.out.println( Arrays.toString( mh_term3 ) );
		System.out.println( "est time: "+getEstimateJoinMH( mh_term1, mh_term2[sampleSearchedList.size()-1], mh_term3[sampleSearchedList.size()-1] ) );
	}
	...
	public double getEstimateNaive( long term1, long term2, double term3 ) {
		return coeff_naive_1 * term1 + coeff_naive_2 * term2 + coeff_naive_3 * term3 * sampleIndexedList.size();
	}

	public double getEstimateJoinMin( long term1, long term2, long term3 ) {
		return coeff_min_1 * term1 + coeff_min_2 * term2 + coeff_min_3 * term3;
	}

	public double getEstimateJoinMH( long term1, long term2, long term3 ) {
	...
	public int findThetaJoinHybridAll( int qSize, int indexK, StatContainer stat, long maxIndexedEstNumRecords, long maxSearchedEstNumRecords, boolean oneSideJoin ) {
		...
		while( currentThreshold <= maxThreshold ) {
			...
			double joinmhEstimation = this.getEstimateJoinMH( mh_term1, mh_term2[tableSearchedSize-1] - (sidx==0?0:mh_term2[sidx-1]), 
			...
			double joinminEstimation = this.getEstimateJoinMin( min_term1, min_term2[tableIndexedSize-1] - (sidx==0?0:min_term2[sidx-1]), 
			...
			double naiveEstimation = this.getEstimateNaive( naive_term1, naive_term2[sidx], naive_term3[sidx] );
			...
			Util.printLog( String.format( "T: %d nT: %d NT: %.2e JT(JoinMin): %.2e TT: %.2e", currentThreshold, nextThreshold,
					naiveEstimation, joinminEstimation, naiveEstimation + joinminEstimation ) );
			Util.printLog( String.format( "T: %d nT: %d NT: %.2e JT(JoinMH): %.2e TT: %.2e", currentThreshold, nextThreshold,
