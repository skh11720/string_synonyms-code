...
//		runOurs();
		runPrevs("JoinPkduckOriginal");
		runPrevs("SIJoinOriginal");
		
		pw.flush();
		pw.close();
	}
	
	private void runOurs() throws IOException, ParseException {
		...
		for ( String dist : distArray ) {
			for ( int delta : deltaArray ) {
				...
				Query query = TestUtils.getTestQuery("NAMES", 0);
				AlgorithmInterface alg = AlgorithmFactory.getAlgorithmInstance(cmd, query.selfJoin);
				StringBuilder strbld = new StringBuilder();
				strbld.append(String.format("%s\t%d\t:", args[1], delta));
				try {
					alg.run(query);
				...
				pw.println(strbld.toString());
				pw.flush();
			}
		}
	}
	
	private void runPrevs(String algName) throws ParseException, IOException {
		String[] args = Arrays.copyOf(argsTemplate, argsTemplate.length);
		args[1] = algName;
		for ( double thres : thresArray ) {
			...
			AlgorithmInterface alg = AlgorithmFactory.getAlgorithmInstance(cmd, query.selfJoin);
			StringBuilder strbld = new StringBuilder();
			strbld.append(String.format("%s\t%.2f\t:", args[1], thres));
			try {
				alg.run(query);
				AlgorithmResultQualityEvaluator.evaluate(alg, query, groundPath);
				alg.getStat().printResult();
