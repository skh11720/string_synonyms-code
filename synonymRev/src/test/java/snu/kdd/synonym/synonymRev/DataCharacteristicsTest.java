...
//		String[] datasetList = {"SPROT", "SYN_D_3.0"};
		...
		String[] arr_attr = {"dataset", "size", "nRule", "DIC", 
				...
				"NAR_min", "NAR_avg", "NAR_max", 
				"NET_min", "NET_avg", "NET_max", 
//				"NT_min", "NT_avg", "NT_max", 
				"NMT_min", "NMT_avg", "NMT_max",
...
//					double[] statNT = getStatNumTrans( query );
...
//						Double.toString( statNT[0] ),
//						Double.toString( statNT[1] ),
//						Double.toString( statNT[2] ),
...
//		ps_dist.close();
	...
	double[] getStatNumTrans( Query query ) {
		double[] output = new double[3];
		output[0] = Double.MAX_VALUE;
		for ( Record rec : query.searchedSet.recordList ) {
			if ( rec.getEstNumTransformed() > DEBUG.EstTooManyThreshold ) continue;
			long net = rec.expandAll().size();
			output[0] = Math.min( output[0], net );
			output[1] += net;
			output[2] = Math.max( output[2], net );
		}
