...
public class SI_Tree_JoinH<K> {
	 ...
	 */
	public HashSet<Pair<Record>> getCandidates( SI_Tree_JoinH<K> o, double threshold ) {
		// Line 1 : Initialize
		...
		for( FenceEntry fe : root.values() ) {
			for( FenceEntry fe_other : o.root.values() ) {
				...
				if( Math.min( fe.v, fe_other.v ) < cut )
					continue;
				...
				for( LeafEntry le : fe.P.values() ) {
					for( LeafEntry le_other : fe_other.P.values() ) {
						...
						// any candidate pair
						if( Math.min( le.t, le_other.t ) < cut )
							continue;
	...
	public List<Record> search( Record r, Iterable<K> sigs, boolean skipChecking ) {
		...
		for( FenceEntry fe : root.values() ) {
			...
			if( !StaticFunctions.overlap( fe.u, fe.v, r.getMinLength(), r.getMaxLength() ) )
				continue;
			...
			for( LeafEntry le : fe.P.values() ) {
				...
				// string
				if( !StaticFunctions.overlap( fe.u, le.t, r.getMinLength(), r.getMaxLength() ) )
					continue;

				// For each signature of r, find all the related records in this index
				for( K sig : sigs ) {
					if( !le.P.containsKey( sig ) )
						continue;
		...
		void add( Record rec, Iterable<K> sigs ) {
			...
			for( K sig : sigs ) {
				if( !P.containsKey( sig ) )
					P.put( sig, new ArrayList<Record>( 5 ) );
				P.get( sig ).add( rec );
				++sigsize;
			}
		}
