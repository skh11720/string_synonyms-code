...
public class TopDownMatrix_DS extends Validator {
	 ...
	 */
	private static boolean getMx( Record x, Record y, int i, int j, Submatch remain ) {
		++recursivecalls;
	 ...
	 * If the value is not computed, compute and then return the value.
	 */
	private static boolean getMy( Record x, Record y, int i, int j, Submatch remain ) {
		++recursivecalls;
		// If this value is already computed, simply return the computed value.
