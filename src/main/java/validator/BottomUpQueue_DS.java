...
public class BottomUpQueue_DS extends Validator {
	...
	private static class QueueEntry {
		...
		public String toString() {
			String str = String.format( "(%d,%d):(%d,%s)", idx1, idx2, type, Arrays.toString( residual ) );
			return str;
		}
		...
		public boolean equals( Object o ) {
			...
			if( me.idx1 != idx1 || me.idx2 != idx2 )
				...
				if( type == 0 )
					return true;
				else if( me.residual.length != residual.length )
					return false;
				boolean rslt = true;
		...
		while( !queue.isEmpty() ) {
			...
			if( qe.type == 0 )
				for( Rule rule1 : r.getApplicableRules( i + 1 ) ) {
					...
					for( Rule rule2 : t.getApplicableRules( j + 1 ) ) {
						...
						// Case b=d
						if( difflen == 0 && StaticFunctions.compare( rule1.getTo(), rule2.getTo() ) == 0 ) {
							next = new QueueEntry( idx1, idx2, qe, 0, null );
							if( rule1.fromSize() == 1 && rule1.getFrom()[ 0 ] == rule2.getFrom()[ 0 ] )
								;
							else
								next.appliedRules = qe.appliedRules + 1;
						}
						// Case d is a prefix of b
						else if( difflen > 0 && StaticFunctions.isPrefix( rule1.getTo(), rule2.getTo() ) ) {
							int[] residual = StaticFunctions.copyIntegerArray( rule1.getTo(), rule2.toSize() );
							next = new QueueEntry( idx1, idx2, qe, 1, residual );
						}
						// Case b is a prefix of d
						else if( difflen < 0 && StaticFunctions.isPrefix( rule2.getTo(), rule1.getTo() ) ) {
							int[] residual = StaticFunctions.copyIntegerArray( rule2.getTo(), rule1.toSize() );
						...
						else if( useLengthFilter && !checkLengthFilter( difflen, r, next.idx1, t, next.idx2 ) )
							continue;
						// Check if we found an answer
						else if( next.idx1 == ( r.size() - 1 ) && next.idx2 == ( t.size() - 1 ) && next.type == 0 )
			...
			// Case l=1
				...
				for( Rule rule : t.getApplicableRules( j + 1 ) ) {
					...
					// Case e=b
					if( difflen == 0 && StaticFunctions.compare( qe.residual, rule.getTo() ) == 0 ) {
						next = new QueueEntry( i, idx2, qe, 0, null );
					}
					// Case b is a prefix of e
					else if( difflen > 0 && StaticFunctions.isPrefix( qe.residual, rule.getTo() ) ) {
						int[] residual = StaticFunctions.copyIntegerArray( qe.residual, rule.toSize() );
						next = new QueueEntry( i, idx2, qe, 1, residual );
					}
					// Case e is a prefix of b
					else if( difflen < 0 && StaticFunctions.isPrefix( rule.getTo(), qe.residual ) ) {
						int[] residual = StaticFunctions.copyIntegerArray( rule.getTo(), qe.residual.length );
					...
					// Check if the next sub-match can be used further
						...
						continue;
					if( rule.fromSize() == 1 && rule.toSize() == 1 && rule.getFrom()[ 0 ] == rule.getTo()[ 0 ] )
						;
					else
						next.appliedRules = qe.appliedRules + 1;
					// Check if we found an answer
					if( next.idx1 == ( r.size() - 1 ) && next.idx2 == ( t.size() - 1 ) && next.type == 0 )
						return next.appliedRules;
					// Check if the next sub-match is already discovered
					else if( !discovered.contains( next ) ) {
						queue.add( next );
	 ...
	 * @return
	 */
	public static boolean checkLengthFilter( int difflen, Record r, int idx1, Record t, int idx2 ) {
		...
		boolean rslt = StaticFunctions.overlap( rCandidateLengths, tCandidateLengths );
		if( !rslt )
			filtered++;
		return rslt;
	}
