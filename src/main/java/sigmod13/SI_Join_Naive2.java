...
public class SI_Join_Naive2 extends SIAlgorithm {
	...
	public SI_Join_Naive2( String DBR_file, String DBS_file, String rulefile ) throws IOException {
	...
	private void buildMap() {
		...
		for( SIRecord rec : tableR ) {
			for( SIRecordExpanded exp : rec.generateAll() ) {
				if( !mapR.containsKey( exp ) )
					mapR.put( exp, new HashSet<SIRecord>() );
				mapR.get( exp ).add( rec );
			}
		}
		for( SIRecord rec : tableS ) {
			for( SIRecordExpanded exp : rec.generateAll() ) {
				if( !mapS.containsKey( exp ) )
					mapS.put( exp, new HashSet<SIRecord>() );
	...
	public void run( double threshold, int filterType ) throws IOException {
		...
		for( SIRecordExpanded exp : mapR.keySet() ) {
			int prefix_size = exp.size() - (int) Math.ceil( threshold * exp.size() ) + 1;
	...
	public void join( double threshold ) {
		...
		for( Map.Entry<Integer, HashSet<SIRecordExpanded>> entry : idxR.entrySet() ) {
			if( !idxS.containsKey( entry.getKey() ) )
				continue;
