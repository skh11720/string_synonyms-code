...
public class Generator {
  ...
  private static void printUsage() {
    ...
    System.out.println(
        "-r <#tokens> <max lhs len> <max rhs len> <#rules>: generate rule");
    System.exit(1);
  }
  ...
  public void genString(int avgLength, int nRecords, File file,
      double equivratio, Rule_ACAutomata atm) throws IOException {
    ...
    while (records.size() < nRecords) {
      if (random.nextDouble() < equivratio) {
        while (true) {
          ...
          Record equivrecord = rec.randomTransform(atm, random)
              .randomTransform(atm, random);
