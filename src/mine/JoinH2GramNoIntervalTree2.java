...
public class JoinH2GramNoIntervalTree2 extends Algorithm {
  ...
  public JoinH2GramNoIntervalTree2(String rulefile, String Rfile, String Sfile)
      throws IOException {
  ...
  private List<IntegerPair> join() {
    ...
    for (Record recS : tableS) {
      ...
      for (int i = 0; i < searchmax; ++i) {
        ...
        for (IntegerPair twogram : available2Grams.get(i)) {
          ...
          for (Record e : tree)
            if (StaticFunctions.overlap(e.getMinLength(), e.getMaxLength(),
                range[0], range[1]))
              list.add(e);
    ...
    System.out.println("Cand extract time : " + candExtractTime + " / "
        + unioncount + " union");
    System.out.println(
        "Join time : " + joinTime + " / " + Validator.checked + " pairs");
    epsilon = ((double) joinTime) / Validator.checked;
    zeta = ((double) candExtractTime) / unioncount;

    System.out
        .println("Usertime vs Wall-clock time : " + joinTime + " / " + tmp);
  ...
  public static void main(String[] args) throws IOException {
    ...
    JoinH2GramNoIntervalTree2 inst = new JoinH2GramNoIntervalTree2(Rulefile,
        Rfile, Sfile);
