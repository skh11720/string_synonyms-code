...
public class IntervalTreeRW<K extends Comparable<K>, V> {
	...
	private class ITNode {
		...
		void leftRotate() {
			...
			y.max = max;
			max = high;
			if( left != nil && max.compareTo( left.max ) < 0 )
				max = left.max;
			if( right != nil && max.compareTo( right.max ) < 0 )
				max = right.max;
		}
		...
		void find( ArrayList<V> rslt, K min, K max ) {
			...
			// node
			if( min.compareTo( high ) <= 0 && low.compareTo( max ) <= 0 )
				rslt.add( value );
			...
			if( right != nil && low.compareTo( max ) <= 0 )
				right.find( rslt, min, max );
		}
