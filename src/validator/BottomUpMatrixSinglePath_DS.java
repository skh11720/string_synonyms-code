...
public class BottomUpMatrixSinglePath_DS extends Validator {
   ...
   */
  private static boolean expandMx(Record x, Record y, int i, int j,
      Submatch remain) {
    ++recursivecalls;
    assert (i <= x.size());
    assert (j <= y.size());
    ...
    else if (Mx[i][j].containsKey(remain)) return false;
    ++niterentry;
    ...
    for (int ridx = 0; ridx < rules.length; ++ridx) {
      ...
      while (isPrefix && rhsidx < rhs.length && remainidx < str.length) {
        if (rhs[rhsidx] != str[remainidx]) isPrefix = false;
        ++rhsidx;
        ++remainidx;
      }
      ...
      if (remainidx == str.length) {
        ...
        if (rhsidx < rhs.length) nextmatch = new Submatch(rule, rhsidx);
   ...
   */
  private static boolean expandMy(Record x, Record y, int i, int j,
      Submatch remain) {
    ++recursivecalls;
    if (remain == EQUALMATCH) return expandEqualMy(x, y, i, j);
    assert (i <= x.size());
    assert (j <= y.size());
    ...
    if (i == x.size())
      return false;
    // If this entry is already extended and evaluated, return false.
    else if (My[i][j].containsKey(remain)) return false;
    ++niterentry;
  ...
  private static boolean expandEqualMy(Record x, Record y, int i, int j) {
    ...
    if (i == x.size())
      return j == y.size();
    // If this entry is already extended and evaluated, return false.
    else if (My[i][j].containsKey(EQUALMATCH)) return false;
    ++niterentry;
    // Check every rule which is applicable to a prefix of x[i..*]
    Rule[] rules = x.getApplicableRules(i);
    assert (rules != null);
