...
public class SI_Tree<T extends RecordInterface & Comparable<T>> {
   ...
   */
  public HashSet<Pair<T>> getCandidates(SI_Tree<T> o, double threshold) {
    // Line 1 : Initialize
    ...
    for (T rec : records) {
      ...
      for (T.Expanded exp : expanded) {
        ...
        for (FenceEntry fe : root.values()) {
          ...
          if (fe.v < exp.size() || exp.size() < fe.u) continue;
          // For all leaf entries
          for (LeafEntry le : fe.P.values()) {
            // Check length condition
            if (le.t < exp.size()) continue;
            ...
            for (T rec2 : values) {
              ...
              // Similarity check
              double sim = rec1.similarity(rec2);
              if (sim == 1) {
  ...
  private class LeafEntry {
    ...
    void add(T rec) {
      ...
      for (int sig : signature) {
        if (!P.containsKey(sig)) P.put(sig, new ArrayList<T>(5));
        P.get(sig).add(rec);
      }
